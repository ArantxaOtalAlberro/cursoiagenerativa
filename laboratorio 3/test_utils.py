...
def test_filtering():
    res = read_csv_to_dict(os.path.join(HERE, 'sample.csv'), filter_column='name', filter_value='Ana')
    assert isinstance(res, list)
    assert len(res) == 1
...
def test_stream_filtered():
    it = read_csv_to_dict(os.path.join(HERE, 'sample.csv'), stream=True, filter_column='name', filter_value='Juan')
    first = next(it)
...
def test_stream_missing_filter_column_raises():
    with pytest.raises(KeyError):
        ...
        list(read_csv_to_dict(os.path.join(HERE, 'sample.csv'), stream=True, filter_column='nonexistent', filter_value='x'))
